...
class UGATIT(object) :
    ...
    def train(self):
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            if self.identity_weight > 0:
                G_loss_A += self.identity_weight * G_identity_loss_A
                G_loss_B += self.identity_weight * G_identity_loss_B
            ...
            if step % self.print_freq == 0:
                ...
                cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'img', 'A2B_%07d_%d.png' % (step, self.device)), A2B * 255.0)
                cv2.imwrite(os.path.join(self.result_dir, self.dataset, 'img', 'B2A_%07d_%d.png' % (step, self.device)), B2A * 255.0)
                self.genA2B.train(), self.genB2A.train(), self.disGA.train(), self.disGB.train(), self.disLA.train(), self.disLB.train()
